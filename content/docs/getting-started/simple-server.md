...
for this line protocol. The `Out` type must accept a `Result` type to represent
errors generated by the service, but for simplicity we'll ignore errors in this
example.
...
#    }
#
fn encode(&mut self, msg: io::Result<String>, buf: &mut Vec<u8>)
         -> io::Result<()>
{
    let msg = msg.expect("Errors are not supported by this protocol");
